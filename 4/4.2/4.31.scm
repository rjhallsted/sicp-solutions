    ...
    (cond ((primitive-procedure? procedure)
        ...
        ((compound-procedure? procedure)
            (eval-sequence
                ...
                (extend-environment
                    (variable-names (procedure-parameters procedure))
                    (list-of-possibly-delayed-args (procedure-parameters procedure) 
                                                    arguments
                                                    env)
...
(define (no-parameters? params) (null? params))
(define (first-parameter p) (car p))
(define (rest-parameters p) (cdr p))
(define (list-of-arg-values exps env)
    (if (no-operands? exps)
        '()
        (cons (actual-value (first-operand exps) env)
            (list-of-arg-values (rest-operands exps) env))))
(define (lazy? param) (and (pair? param) (eq? (cadr param) 'lazy)))
(define (lazy-memo? param) (and (pair? param) (eq? (cadr param) 'lazy-memo)))
(define (determine-arg parameter exp env)
    (cond ((lazy? parameter)
            (delay-it exp env))
        ((lazy-memo? parameter)
            (delay-and-memo-it exp env))
        (else
            (actual-value exp env))))
(define (list-of-possibly-delayed-args procedure-params exps env)
    (if (or (no-operands? exps) (no-parameters? procedure-params))
        '()
        (let ((arg (determine-arg (first-parameter procedure-params)
                                  (first-operand exps)
                                  env)))
            (cons arg
                 (list-of-possibly-delayed-args (rest-parameters procedure-params)
                                                (rest-operands exps)
                                                env)))))

(define (variable-names variables)
    (if (null? variables)
        '()
        (let ((var (if (pair? (car variables))
                        (caar variables)
                        (car variables))))
            (cons var (variable-names (cdr variables))))))
...
(define (delay-it exp env)
    (list 'thunk exp env))
(define (delay-and-memo-it exp env)
...
(define (make-frame variables values)
    (cons 'frame
        (pair-merge-lists (variable-names variables) values)))
